...
#     在 Android 16 (16KB 页内核，如 Pixel 9/10) 上加载即崩。5.44.0 已为 16KB 对齐。
if "im_libcore:5.44.0" not in g:
    g2, n = re.subn(r"im_libcore:[\d.]+", "im_libcore:5.44.0", g)
    if n:
        changed.append(f"im_libcore 升为 5.44.0（{n} 处）")
    g = g2
if "im_chatroom:5.44.0" not in g:
    g2, n = re.subn(r"im_chatroom:[\d.]+", "im_chatroom:5.44.0", g)
    if n:
        changed.append(f"im_chatroom 升为 5.44.0（{n} 处）")
    g = g2
